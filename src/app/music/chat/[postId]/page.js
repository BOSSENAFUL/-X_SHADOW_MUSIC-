      ...
      <SidebarProvider>
        ...
        <SidebarInset className="md:ml-0 overflow-y-auto overflow-x-hidden h-svh relative flex flex-col bg-zinc-950">
          <div className="flex flex-col items-center justify-center flex-1 space-y-4">
            ...
            <p className="text-zinc-500 animate-pulse">Loading discussion detail...</p>
          </div>
        </SidebarInset>
      </SidebarProvider>
    );
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-3">
                <Avatar className="w-9 h-9 border border-white/5">
                  ...
                  <AvatarFallback className="bg-zinc-900 text-xs">{post.author?.name?.[0]?.toUpperCase()}</AvatarFallback>
                </Avatar>
                <div className="flex flex-col">
                  <span className="font-bold text-sm text-zinc-100">{post.author?.name}</span>
                  <div className="flex items-center gap-2 text-[10px] md:text-xs text-zinc-500 font-medium">
                  ...
                  {session?.user?.id === post.author?._id && (
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <button 
                          disabled={isDeleting}
                          className="flex items-center gap-1.5 text-xs font-semibold text-zinc-500 hover:text-destructive transition-colors ml-2"
                          title="Delete post"
                        >
                      ...
                      <AlertDialogContent className="bg-zinc-950 border-zinc-800">
                        <AlertDialogHeader>
                          <AlertDialogTitle className="text-zinc-50">Delete Post</AlertDialogTitle>
                          <AlertDialogDescription className="text-zinc-400">
                            Are you sure you want to delete this post? This action cannot be undone and will remove all associated comments.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel className="bg-transparent text-zinc-300 hover:bg-zinc-900 border-zinc-800">Cancel</AlertDialogCancel>
                          <AlertDialogAction 
              ...
              <h1 className="text-xl md:text-2xl font-bold text-zinc-50 tracking-tight leading-tight">
                {post.title}
              </h1>
            </div>

            <Card className="bg-transparent border-zinc-800/60 overflow-hidden rounded-xl">
              <CardContent className="p-4 md:p-6 lg:p-8">
                <p className="text-sm md:text-base text-zinc-300 leading-relaxed whitespace-pre-wrap font-medium">
                  {post.content}
                </p>
              </CardContent>
              <CardFooter className="bg-zinc-900/10 border-t border-zinc-800/40 p-3 md:p-4 px-4 md:px-6 flex items-center justify-between">
                <div className="flex items-center gap-6 md:gap-8">
                  <button 
                    onClick={handleLike}
                    className={`flex items-center gap-2 transition-colors font-bold text-[10px] md:text-xs uppercase tracking-wider hover:text-white ${post.likes?.some(id => id === session?.user?.id) ? 'text-primary' : 'text-zinc-500'}`}
                  >
                  ...
                  <div className="flex items-center gap-2 text-zinc-500 font-bold text-[10px] md:text-xs uppercase tracking-wider">
                    ...
                    <div key={comment._id} className="space-y-4">
                      ...
                      <div className="flex gap-3 md:gap-4">
                        <Avatar className="w-8 h-8 md:w-10 md:h-10 shrink-0">
                          ...
                          <AvatarFallback className="bg-zinc-800 text-xs">{comment.author?.name?.[0]?.toUpperCase()}</AvatarFallback>
                        </Avatar>
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-semibold text-sm text-zinc-200">{comment.author?.name}</span>
                          ...
                          <div className="flex items-center gap-4 text-xs font-semibold text-zinc-500 pt-1">
                            <button 
                              ...
                              className={`flex items-center gap-1.5 transition-colors hover:text-zinc-300 ${comment.likes?.includes(session?.user?.id) ? 'text-white' : ''}`}
                            >
                          ...
                          {replyingTo === comment._id && (
                            <div className="mt-4 flex flex-col gap-3 animate-in fade-in duration-200">
                              <Textarea 
                                ...
                                className="bg-transparent border-0 border-b border-zinc-800 rounded-none px-0 py-2 focus-visible:ring-0 focus-visible:border-zinc-500 resize-none min-h-[40px] text-sm text-zinc-200"
                                value={replyContent}
                                onChange={(e) => setReplyContent(e.target.value)}
                                autoFocus
                              />
                              <div className="flex gap-2 justify-end">
                                <Button size="sm" variant="ghost" onClick={() => setReplyingTo(null)} className="h-7 text-xs font-semibold rounded-full px-4 text-zinc-400 hover:text-white">Cancel</Button>
                          ...
                          {replies.map((reply) => (
                            <div key={reply._id} className="flex gap-3">
                              <Avatar className="w-6 h-6 shrink-0 mt-0.5">
                                ...
                                <AvatarFallback className="bg-zinc-800 text-[10px]">{reply.author?.name?.[0]?.toUpperCase()}</AvatarFallback>
                              </Avatar>
                              <div className="flex-1 space-y-1">
                                <div className="flex items-center gap-2">
                                  <span className="font-semibold text-xs md:text-sm text-zinc-200">{reply.author?.name}</span>
                                  <span className="text-xs text-zinc-500">
                                ...
                                <div className="flex items-center gap-4 text-xs font-semibold text-zinc-500 pt-1">
                                  <button 
                                    ...
                                    className={`flex items-center gap-1.5 transition-colors hover:text-zinc-300 ${reply.likes?.includes(session?.user?.id) ? 'text-white' : ''}`}
                                  >
