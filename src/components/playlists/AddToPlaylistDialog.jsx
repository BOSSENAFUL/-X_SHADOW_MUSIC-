    ...
    try {
      ...
      if (createResult.success) {
        ...
        if (addResult.success) {
          ...
          toast.success(`Song added to new playlist: ${createResult.data.name}`);
          // Close dialog
          onOpenChange(false);
        } else {
          console.error('Failed to add song to new playlist:', addResult.error);
          toast.error('Failed to add song to new playlist');
        }
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-[500px] h-[85vh] max-h-[600px] p-0 gap-0 flex flex-col overflow-hidden z-100">
        ...
        {/* Search */}
        <div className="shrink-0 p-4 sm:p-6 pb-3 sm:pb-4">
        ...
        <div className="shrink-0 px-4 sm:px-6 pb-3">
          ...
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-primary/10 flex items-center justify-center mr-3 shrink-0">
              {creatingPlaylist ? (
                  ...
                  {filteredPlaylists.map((playlist) => {
                    ...
                    return (
                      <Button
                        key={playlist._id}
                        variant="ghost"
                        className={`w-full justify-start h-14 sm:h-16 px-3 sm:px-4 rounded-lg transition-all duration-200 ${songInPlaylist
                      ...
                      >
                        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center mr-3 sm:mr-4 shrink-0 transition-colors ${songInPlaylist
